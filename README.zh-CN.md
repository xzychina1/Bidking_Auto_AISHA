...
下面的图导出自 [docs/architecture/](docs/architecture) 里的 draw.io 源文件。每条箭头都标注了**它传递的是什么**，组件也按所属泳道分组。
...
下面是一次真实的端到端运行长什么样。每段 GIF 都放在它对应的步骤旁；这些片段是同一次连续会话录下来的，合起来覆盖：启动 → 出价 → 对局结束。
