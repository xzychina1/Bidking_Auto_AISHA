...
每条箭头都标注了**它传递的是什么**，所以这张图是数据流，而不只是连线图。组件按职责分组：截图/OCR、估值、编排。

```mermaid
flowchart TD
  subgraph GAME["BidKing 游戏（窗口化）"]
    GW["游戏窗口"]
  end

  subgraph CAP["截图与 OCR — bidking_maa_test"]
    WB["window_backend（查找 + 抓取客户区）"]
    OCR["rapidocr / onnxruntime（图像转文字）"]
    PAR["central_info_parser（文字转事实）"]
  end

  subgraph VAL["估值 — manual_bidking_advisor + shadow"]
    ADV["出价顾问（价格模型 + 权重）"]
    SHA["shadow 桥接（getlog 单件价值）"]
    RISK["激进度 + 上限 + 安全开关（最终出价）"]
  end

  subgraph ORC["编排"]
    GUI["bidking_gui（Tkinter 控件 + 日志）"]
    LOOP["fresh_bidking_bot（状态机 + 轮询循环）"]
    ACT["pyautogui（点击 + 键盘）"]
  end
  ...
  GW -- "客户区截图" --> WB
  WB -- "缩放后的 ROI 裁剪图（PNG）" --> OCR
  OCR -- "原始文字" --> PAR
  PAR -- "回合号 + 拍卖事实" --> ADV
  SHA -- "单件期望价值" --> ADV
  ADV -- "建议价格（单位：万 / w）" --> RISK
  CFG -- "权重 + 单格价格" --> ADV
  CFG -- "坐标、上限、计时" --> LOOP
  RISK -- "封顶后的最终出价" --> LOOP
  GUI -- "开启/停止 + 设置" --> LOOP
...
可以理解为五步：**截图**窗口 → 对中央信息区做 **OCR + 解析** → 给这件拍品**估值**（顾问，可由 shadow 补充）→ 套用**激进度/上限** → 用鼠标键盘**执行**，循环直到达到设定的重复次数。
...
- 1920×1080 的游戏窗口最贴合默认坐标（其它尺寸会自动缩放，见[针对你的屏幕做校准](#针对你的屏幕做校准)）
...
下面是一次真实的端到端运行长什么样。每段 GIF 都放在它对应的步骤旁，日志行是 GUI 的 **运行日志 / Debug** 面板里有代表性的输出（时间戳做了简化）。这些片段是同一次连续会话录下来的，合起来覆盖：启动 → 出价 → 对局结束。
...
`config.json` 里每一个点击点和截图区域都按 **1920×1080 参考分辨率**（`window.reference_client_size`）存储，运行时会**自动缩放**到你的真实窗口大小。所以窗口尺寸不同，往往**不用改任何东西**也能用。如果点击位置不对，可以这样校准：

1. **看每个点击实际会落在哪里。** 下面这条命令会针对你当前的窗口，把每个命名点击打印成 `name: config=(x,y) origin=... -> screen=(x,y)` 然后退出——拿它和实际游戏对照：
...
2. **检查 OCR 区域读到了什么。** 截一张游戏图，然后把所有区域框叠加到图上：

   ```powershell
   python .\bidking_maa_test\analyze_screenshot.py --image C:\path\to\screenshot.png
   ```

3. **抓取运行中每个区域实际截到的内容**：在 `config.json` 里打开调试输出；裁剪图和每回合数据包会写到 `bidking_fresh_bot\runs\` 下：

   ```json
   "debug": { "save_crops": true, "save_round_debug": true }
   ```

4. **调整数值。** 地图入口点在 `automation.maps.<n>.point`，各动作点击在 `clicks.*`，OCR 区域在 `capture.*`。大多数点用左上角原点；部分用 `"origin": "left_bottom"`（`y` 从底部算起）。改的时候保持在 1920×1080 参考坐标系里——缩放到实际窗口的事交给程序。

## 手动计算器

**手动计算器** 标签页跑的是和机器人相同的价格模型，只不过输入是你手填的数字——适合在不开实时运行的情况下，检查某个出价"应该"是多少。填入回合、角色，以及各颜色的件数/格子/价格，点 **计算**，它会打印出明细。**同步自动化设置** 会把当前角色和权重复制过来，让计算器和机器人保持一致。
...
通常是捕获了错误的窗口，或者游戏界面和默认区域不一致。先检查窗口标题、分辨率和缩放，并用上面的校准工具确认截图区域是否对齐。
...
不必。默认坐标是为 1920×1080 调的，但坐标会自动缩放到其它尺寸。差异很大的布局可能仍需按上面的步骤重新校准。
